...
│   └── chat/             # Chat con IA
├── agent_ia_core/        # Motor de IA (ver estructura detallada abajo)
...
│   ├── xml_parser.py           # Parser de eForms XML
│   ├── chunking.py             # Chunking de documentos
│   └── tools_xml.py            # XPath lookup tools
├── prompts/                    # System prompts del agente
│   └── prompts.py
├── indexing/                   # RAG y vectorización
│   ├── retriever.py            # HybridRetriever (ChromaDB)
│   ├── index_build.py          # Construcción de índices
│   └── ingest.py               # Ingesta de datos
├── download/                   # Descarga de licitaciones
│   ├── descarga_xml.py         # Descarga desde TED API
│   └── token_tracker.py        # Tracking de tokens/costos
...
|------|-----------|-------------|
| `data/xml/` | XMLs de TED | Licitaciones descargadas de TED API |
| `data/index/chroma/` | ChromaDB | Base de datos vectorial para RAG |
| `data/records/` | JSON records | Registros parseados de licitaciones |
| `logs/chat/` | Logs de chat | Conversaciones completas (input/output LLM) |
| `logs/indexacion/` | Logs de indexación | Proceso de vectorización |
| `logs/obtener/` | Logs de descarga | Descargas desde TED API |
| `db.sqlite3` | Base de datos | Django ORM (usuarios, licitaciones, chat) |
| `static/` | Estáticos | CSS, JS, imágenes |
| `media/` | Media | Archivos subidos por usuarios |
...
**🎯 Empieza aquí:**
- **[docs/DOCS_INDEX.md](docs/DOCS_INDEX.md)** - Índice completo de documentación, guías por rol

**📖 Documentación principal:**
- **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** - Arquitectura completa del sistema v3.7.1
- **[docs/TOOLS_REFERENCE.md](docs/TOOLS_REFERENCE.md)** - Referencia completa de las 16 tools
- **[docs/FLUJO_EJECUCION_CHAT.md](docs/FLUJO_EJECUCION_CHAT.md)** - Flujo de ejecución paso a paso con Review Loop
- **[docs/CONFIGURACION_AGENTE.md](docs/CONFIGURACION_AGENTE.md)** - Configuración detallada del agente

---

**TenderAI Platform v3.7.1** - Encuentra las mejores oportunidades de licitación con IA
