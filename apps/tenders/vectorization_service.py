...
class VectorizationService:
    ...
    def index_all_tenders(self, progress_callback=None, cancel_flag_checker=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # SOLUCIÓN ROBUSTA PARA WINDOWS: Usar ChromaDB API para eliminar colección
            # Esto evita WinError 32 (archivo en uso) y errores de "collection already exists"
            ...
            # PASO 1: Intentar eliminar la colección usando ChromaDB API
            if chroma_path.exists():
                try:
                    # Crear cliente temporal para eliminar la colección
                    temp_client = chromadb.PersistentClient(path=persist_dir)

                    # Verificar si la colección existe
                    existing_collections = [col.name for col in temp_client.list_collections()]

                    if collection_name in existing_collections:
                        # Eliminar usando API (cierra conexiones correctamente)
                    ...
                    # IMPORTANTE: Cerrar cliente y forzar liberación de archivos
                    del temp_client
                    import gc
                    import time
                    gc.collect()
                    time.sleep(1.0)  # Dar tiempo a Windows para liberar handles

                    # PASO 2: Ahora intentar eliminar el directorio (opcional, solo si es seguro)
                    # Si falla, no es crítico porque ya eliminamos la colección
                    try:
                        import shutil
                        ...
                        if progress_callback:
                            progress_callback({
                                'type': 'info',
                                'message': 'Índice anterior eliminado completamente'
                            })
                    except Exception as dir_error:
                        # No es crítico: la colección ya fue eliminada
                        if progress_callback:
                            progress_callback({
                                'type': 'info',
                                'message': 'Colección eliminada (directorio será reutilizado)'
                            })

                except Exception as e:
                    # Si falla la eliminación API, informar pero continuar
                    if progress_callback:
                        progress_callback({
                            'type': 'warning',
                            'message': f'No se pudo eliminar colección anterior: {str(e)}'
                        })
            else:
                if progress_callback:
                    progress_callback({
            ...
            # PASO 3: Crear cliente (ahora sin conflictos)
            client = chromadb.PersistentClient(path=persist_dir)
            ...
            # Create or get collection (usar get_or_create para mayor robustez)
            if progress_callback:
                progress_callback({
                    'type': 'info',
    ...
    def clear_vectorstore(self, force: bool = False) -> Dict[str, Any]:
        """
        Clear the entire vectorstore by deleting collections using ChromaDB API.

        This method uses ChromaDB API to properly close connections before deletion,
        preventing WinError 32 on Windows.
        ...
        try:
            ...
            # PASO 1: Usar ChromaDB API para eliminar colección
            try:
                client = chromadb.PersistentClient(path=persist_dir)
                ...
                # Cerrar cliente y liberar archivos
                del client
                gc.collect()
                time.sleep(1.0)  # Dar tiempo a Windows para liberar handles

                # PASO 2: Intentar eliminar directorio
                try:
                    shutil.rmtree(chroma_path)
                    return {
                        'success': True,
                    ...
                    # No es crítico: las colecciones ya fueron eliminadas
                    return {
                        'success': True,
                        'message': f'Colecciones eliminadas correctamente (directorio será reutilizado)'
                    }

            except Exception as e:
                return {
                    'success': False,
                    'error': f'Error al eliminar vectorstore: {str(e)}'
                }
